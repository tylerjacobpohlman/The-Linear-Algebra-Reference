...
$W$ is not a subspace due to [property (3](3.5%20Subspaces,%20Basis,%20Dimension,%20and%20Rank.md#definition-subspace)%20of%20subspaces), **closure under scalar multiplication**:
...
Following the [definition of a subspace](3.5%20Subspaces,%20Basis,%20Dimension,%20and%20Rank.md#definition-subspace), show the following:
...
Let $A$ be an *m x n* matrix. the **row space**, denoted $Row(A)$, is the [subspace](3.5%20Subspaces,%20Basis,%20Dimension,%20and%20Rank.md#definition-subspace) of $\mathbb{R}^{n}$ spanned by the rows of $A$. The **column space**, denoted $Col(A)$, is the [subspace](3.5%20Subspaces,%20Basis,%20Dimension,%20and%20Rank.md#definition-subspace) of $\mathbb{R}^{n}$ spanned by the columns of $A$.
...
Following the [definition of a basis](3.5%20Subspaces,%20Basis,%20Dimension,%20and%20Rank.md#definition-basis), first setup a matrix $A$ whose columns are that of $B$.
...
Recall the [theorem for row equivalence](3.5%20Subspaces,%20Basis,%20Dimension,%20and%20Rank.md#theorem-row-equivalence), *if $R$ is a row echelon form of $A$, then $Row(A)=Row(R)$.* As such, use Gauss-Jordan Elimination to find the reduced row echelon form of $A$.
...
Let $S$ be a [subspace](3.5%20Subspaces,%20Basis,%20Dimension,%20and%20Rank.md#definition-subspace) of $\mathbb{R}^{n}$. Any two bases for $S$ have the same number of vectors.

## \#Definition Dimension

If $S$ is a [subspace](3.5%20Subspaces,%20Basis,%20Dimension,%20and%20Rank.md#definition-subspace) of $\mathbb{R}^{n}$, then the number of vectors in a [basis](3.5%20Subspaces,%20Basis,%20Dimension,%20and%20Rank.md#definition-basis) for $S$ is called the **dimension** of $S$, denoted $dim(S)$.

### \#Example $dim(\vec{0})=0$ because $\vec{0}$ has no basis.

## \#Definition Rank

The **rank** of a matrix $A$, denoted $rank(A)$, is the dimension of its [row space and column space](3.5%20Subspaces,%20Basis,%20Dimension,%20and%20Rank.md#definition-row-space-and-column-space).
...
The **nullity** of a matrix $A$, denoted $nullity(A)$, is the [dimension](3.5%20Subspaces,%20Basis,%20Dimension,%20and%20Rank.md#definition-dimension) of its [null space](3.5%20Subspaces,%20Basis,%20Dimension,%20and%20Rank.md#definition-null-space).
...
This theorem builds off of [The Fundamental Theorem of Invertible Matrices: Version 1](3.3%20The%20Inverse%20of%20a%20Matrix.md#theorem-the-fundamental-theorem-of-invertible-matrices-version-1), adding 8 more equivalency statements.
